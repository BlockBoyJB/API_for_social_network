...
async def add_reaction(
    new_reaction: ReactionCreate, session: AsyncSession = Depends(get_async_session)
):
    ...
    stmt = (
        update(User)
        .where(User.username == new_reaction.username)
        .values(total_reactions=User.total_reactions + 1)
    )
    ...
    stmt = (
        update(Post)
        .where(Post.title == new_reaction.title, Post.username == new_reaction.username)
        .values(post_reactions=Post.post_reactions + 1)
    )
    await session.execute(stmt)
    await session.commit()
