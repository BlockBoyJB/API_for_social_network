...
async def graph_leaderboard(session: AsyncSession = Depends(get_async_session)):
    query = select(User.username, User.total_reactions).order_by(desc(User.total_reactions))
...
@router.get("/list")
async def list_leaderboard(sort: str, session: AsyncSession = Depends(get_async_session)):
    if sort == "desc":
        query = select(User.username, User.total_reactions).order_by(desc(User.total_reactions))

    else:
        query = select(User.username, User.total_reactions).order_by(User.total_reactions)
